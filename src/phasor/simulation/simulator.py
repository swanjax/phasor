...
class Simulator:
    ...
    def _topological_sort(self) -> list[Component]:
        ...
        queue = deque(
            component
            for component, indegree in indegrees.items()
            if indegree == 0
        )
    ...
    def run(self) -> dict[Detector, float]:
        ...
        for component in execution_order:
            if isinstance(component, Laser):
                ...
                component.output_ports[0].waveguide.field = field

            elif isinstance(component, PhaseShifter):
                input_field = component.input_ports[0].waveguide.field
                if input_field is None:
                    raise RuntimeError(
                        f"No optical field at {component.input_ports[0]}"
                    )
                output_field = component.transform(field=input_field)
                component.output_ports[0].waveguide.field = output_field
                ...
                upper_input_field = upper_input_guide.field if upper_input_guide else OpticalField(0j)
                lower_input_field = lower_input_guide.field if lower_input_guide else OpticalField(0j)
