...
def test_phase_shifter():
    ...
    waveguideLP =  circuit.connect(
        laser.output_port,
        phase_shifter.input_port
    )
    waveguidePD = circuit.connect(
        phase_shifter.output_port,
        detector.input_port
    )
...
def test_beam_splitter():
    ...
    waveguide1 = circuit.connect(
        laser.output_port,
        beam_splitter.input_ports[0]
    )
    waveguide2 = circuit.connect(
        beam_splitter.output_ports[0],
        detect1.input_port
    )
    waveguide3 = circuit.connect(
        beam_splitter.output_ports[1],
        detect2.input_port
    )
    simulator = Simulator(circuit)
    results = simulator.run()
